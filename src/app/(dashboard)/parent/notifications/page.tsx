...
import { Bell, CheckCircle2, ShieldAlert, Star } from "lucide-react";
...
export default function NotificationsPage() {
  ...
  const [notifications, setNotifications] = useState([
    { id: 1, title: 'تحديث بجدول الاختبارات', desc: 'تم الإعلان عن جدول اختبارات نهاية الفصل الدراسي الأول لجميع الأبناء.', time: 'منذ ساعتين', type: 'info', isNew: true },
    { id: 2, title: 'تذكير: فاتورة مستحقة', desc: 'نقاط التنبيه: يرجى تسديد الدفعة الثالثة من القسط الدراسي للابن عمر.', time: 'أمس', type: 'warning', isNew: true },
    { id: 3, title: 'نتيجة التقييم الشهري', desc: 'توفرت الآن نتيجة التقييم الشهري للابن خالد. يمكنك النقر لعرض الدرجات.', time: 'منذ يومين', type: 'success', isNew: false },
    { id: 4, title: 'حدث جديد: يوم المهنة', desc: 'الأكاديمية تنظم يوم المهنة القادم وندعوكم للحضور.', time: 'الأسبوع الماضي', type: 'star', isNew: false },
  ]);
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-2">
        <div>
          <h1 className="text-2xl font-black text-slate-900 dark:text-white">التنبيهات</h1>
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">سجل التنبيهات والأحداث الخاصة بالأبناء والمدرسة</p>
        </div>
        <button
      ...
      <div className="glass-card overflow-hidden">
        <div className="divide-y divide-slate-100 dark:divide-white/5">
          {notifications.map((notif) => (
            <div key={notif.id} className={`p-6 transition-colors hover:bg-slate-50 dark:hover:bg-white/5 cursor-pointer relative ${notif.isNew ? 'bg-primary/5 dark:bg-primary/10' : ''}`}>

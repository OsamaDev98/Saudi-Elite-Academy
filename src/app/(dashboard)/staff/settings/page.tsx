...
import { User, Lock, Bell, Globe, Database, Save, Activity, LayoutTemplate, CheckCircle2 } from "lucide-react";
    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-2">
        <div>
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">إدارة ملفك التعريفي كمعلم وتخصيص تجربة التصفح</p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-1 space-y-2">
          <div className="glass-card p-4 space-y-1">
            ...
            <button 
              ...
              className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl font-bold transition-colors ${activeTab === 'grading' ? 'bg-primary/10 text-primary' : 'hover:bg-slate-50 dark:hover:bg-white/5 text-slate-600 dark:text-slate-300'}`}
            >
              <LayoutTemplate className="w-5 h-5" />
              تفضيلات الرصد والتقييم
            </button>
            <button 
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="glass-card p-6 sm:p-8">
              ...
              <>
                ...
                <div className="flex flex-col sm:flex-row gap-8 mb-8">
                  <div className="w-32 h-32 rounded-2xl overflow-hidden relative bg-slate-100 dark:bg-white/5 ring-4 ring-primary/20 shrink-0">
                    <Image src="https://images.unsplash.com/photo-1544717297-fa95b6ee9643?w=400&auto=format&fit=crop&q=60" alt="Teacher Profile" fill className="object-cover" />
                  ...
                  <div className="flex-1 space-y-4">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <div className="space-y-1">
                        ...
                        <input type="text" defaultValue="أحمد" className="mt-2 w-full bg-slate-50 dark:bg-black/20 border border-slate-200 dark:border-white/10 rounded-xl px-4 py-2.5 text-slate-900 dark:text-white focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary transition-colors" />
                      </div>
                      <div className="space-y-1">
                        <label className="text-sm font-bold text-slate-600 dark:text-slate-300">الاسم الأخير</label>
                        <input type="text" defaultValue="خالد" className="mt-2 w-full bg-slate-50 dark:bg-black/20 border border-slate-200 dark:border-white/10 rounded-xl px-4 py-2.5 text-slate-900 dark:text-white focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary transition-colors" />
                      </div>
                    </div>
                    <div className="space-y-1">
                      <label className="text-sm font-bold text-slate-600 dark:text-slate-300">البريد الإلكتروني المدني (للأكاديمية)</label>
                      <input type="email" defaultValue="ahmed.k@teachers.saudielite.edu" disabled className="mt-2 w-full bg-slate-100 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-xl px-4 py-2.5 text-slate-500 dark:text-slate-400 cursor-not-allowed" />
                    </div>
                  </div>
                </div>
                ...
                <h2 className="text-xl font-black text-slate-900 dark:text-white mb-6 mt-10 flex items-center gap-2 border-t border-slate-100 dark:border-white/10 pt-8">
                  المعلومات الإضافية والمسار
                </h2>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                  <div className="space-y-1">
                    <label className="text-sm font-bold text-slate-600 dark:text-slate-300">التخصص الدراسي</label>
                    <input type="text" defaultValue="الرياضيات البحتة والتطبيقية" className="mt-2 w-full bg-slate-50 dark:bg-black/20 border border-slate-200 dark:border-white/10 rounded-xl px-4 py-2.5 text-slate-900 dark:text-white focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary transition-colors" />
                  </div>
                  <div className="space-y-1">
                    <label className="text-sm font-bold text-slate-600 dark:text-slate-300">رقم الهاتف للطلاب/الآباء (اختياري)</label>
                    <input type="tel" defaultValue="050 987 6543" className="mt-2 w-full bg-slate-50 dark:bg-black/20 border border-slate-200 dark:border-white/10 rounded-xl px-4 py-2.5 text-slate-900 dark:text-white focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary transition-colors text-left" dir="ltr" />
                  </div>
                  <div className="space-y-1 sm:col-span-2">
                    <label className="text-sm font-bold text-slate-600 dark:text-slate-300">نبذة تعريفية سريعة</label>
                    <textarea rows={4} placeholder="معلم رياضيات بخبرة تزيد عن 8 سنوات في تدريس المرحلة الثانوية. مهتم بتطبيق استراتيجيات التعلم النشط وتطوير مهارات التفكير المنطقي لدى الطلاب." className="mt-2 w-full bg-slate-50 dark:bg-black/20 border border-slate-200 dark:border-white/10 rounded-xl px-4 py-3 text-slate-900 dark:text-white focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary transition-colors resize-none"></textarea>
                  </div>
                </div>
              </>
            )}
            ...
            {activeTab === 'grading' && (
              <section className="text-center py-10">
                <LayoutTemplate className="w-12 h-12 mx-auto text-slate-300 dark:text-slate-600 mb-4" />
                <h3 className="text-lg font-bold text-slate-900 dark:text-white">تفضيلات الرصد والتقييم</h3>
